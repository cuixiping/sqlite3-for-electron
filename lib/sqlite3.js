...
function getBindingPath() {
	...
	var arch = os.arch();
	var ver = process.versions.electron;
	var binding_path;
	if (process.versions.electron) {
		ver = ver.replace(/\.\d+$/, '');
		binding_path = path.resolve(__dirname, `binding/electron-v${ver}-${platform}-${arch}/node_sqlite3.node`);
	} else if (process.versions.modules) {
		binding_path = path.resolve(__dirname, `binding/node-v${ver}-${platform}-${arch}/node_sqlite3.node`);
	} else {
		throw Error('sqlite3-for-electron cannot run out of electron or node');
	}
	return binding_path;
}

var binding_path = getBindingPath();
if (!fs.existsSync(binding_path)) {
	throw Error('node_sqlite3.node not exists in ' + path.dirname(binding_path));
}
